# bpertussisciwgs: Usage

## Introduction

Basic usage and parameter description below. Default parameter definitions can be found in [nextflow.config](../nextflow.config).  

## Samplesheet input

You will need to create a samplesheet with information about the samples you would like to analyse with the pipeline and specify its location with `--input`. The required format is a 3-column, comma-separated file with header as shown in the examples below.
...
The provided `sample` identifiers must be identical when re-sequencing the same sample more than once e.g. to increase sequencing depth. The pipeline will concatenate the raw reads before performing any downstream analysis. Below is an example for the same sample sequenced across 3 lanes:
...
> **IMPORTANT:** The **bpertussisciwgs** workflow is intended for paired-end Illumina reads only and has not been tested using single-end or other inputs.
...
### Output directory
Define a location for the pipeline results. See [output](./output.md) for specific contents and organization.  
...
### Reference genomes
Read filtering requires definition of two reference genomes:  

1. Human reference for negative selection.  One is not included due to large file size, but users may select their own reference, including those from the [Genome Reference Consortium](https://www.ncbi.nlm.nih.gov/grc/human). The **bpertussisciwgs** workflow was developed primarily with GRCh38.  
```console
--hsref '[path to human genome reference bowtie2 index]'
```
2. *B. pertussis* reference genome for positive selection, ideally a complete assembly that is phylogenetically related to the expected target strain. A copy of [CP013078](https://www.ncbi.nlm.nih.gov/nuccore/CP013078.1/) is provided in [assets](/assets/).
```console
--fasta '[path to B. pertussis genome reference fasta]'
```

### *Optional:* Allele typing scheme
A FASTA file of typing gene targets for reporting per-locus coverage depth as an additional assessment of target genome recovery. An example wgMLST scheme FASTA is provided in [assets](../assets/) and other schemes are available from [Institut Pasteur](https://bigsdb.pasteur.fr/bordetella/).  (default: null) 

```console
--mlst '[path to typing scheme fasta]'
```


### *Optional:* De novo assembly with Skesa  
Filtered *B. pertussis* reads can be assembled with Skesa and contigs checked with QUAST as an additional assessment of target genome recovery. (defualt: false)  
...
```console
nextflow run bpertussisciwgs/main.nf --input samplesheet.csv --genome GRCh38 -profile singularity --fasta NC_002929.fasta --outdir myresults
```
...
First, go to the [bpertussisciwgs releases page](https://github.com/cdcgov/bpertussis-ciwgs/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.

This version number will be logged in reports when you run the pipeline (`{outdir}/pipeline_info/software_versions.yml`), so that you'll know what you used when you look back in the future.
...
> **Note:** We specify just the process name i.e. `STAR_ALIGN` in the config file and not the full task name string that is printed to screen in the error message or on the terminal whilst the pipeline is running i.e. `RNASEQ:ALIGN_STAR:STAR_ALIGN`. You may get a warning suggesting that the process selector isn't recognised but you can ignore that if the process name has been specified correctly. This is something that needs to be fixed upstream in core Nextflow.
...
> **Note:** If you wish to periodically update individual tool-specific results (e.g. Pangolin) generated by the pipeline then you must ensure to keep the `work/` directory otherwise the `-resume` ability of the pipeline will be compromised and it will restart from scratch.
